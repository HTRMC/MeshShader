...
const int DEFAULT_SERVER_PORT = 42069;
...
// Uniform buffer object - same as in the mesh_shader.glsl
...
private:
    ...
    std::thread networkUpdateThread;
    ...
    // Input state
    bool keyPressed[348] = {false}; // GLFW key count
    ...
    void networkUpdateLoop()
    {
        ...
        const milliseconds updateInterval(100);
        ...
        while (running)
        {
            ...
            // Check if we need to send position update
            {
                ...
                if (gameState.connected && gameState.positionChanged)
                {
                    sendPositionUpdate();
                    gameState.positionChanged = false;
                }
            }
        }
    }

    void handleConnectResponse(const NetworkMessage& msg)
    {
        std::lock_guard<std::mutex> lock(gameStateMutex);

        gameState.localPlayerId = msg.clientId;
        gameState.connected = true;

        std::cout << "Connected to server with Player ID: " << gameState.localPlayerId << std::endl;
    }

    void handleFullState(const NetworkMessage& msg)
    {
        std::lock_guard<std::mutex> lock(gameStateMutex);

        // Update local game state with server's data
        for (int i = 0; i < MAX_PLAYERS; i++)
        {
            gameState.players[i].active = msg.payload.fullState.playerActive[i];

            // Only update positions for other players, not our own position
            if (gameState.players[i].active)
            {
                // Extract position
                gameState.players[i].position.x = msg.payload.fullState.positions[i * 3];
                gameState.players[i].position.y = msg.payload.fullState.positions[i * 3 + 1];
                gameState.players[i].position.z = msg.payload.fullState.positions[i * 3 + 2];

                // Extract color
                gameState.players[i].color.r = msg.payload.fullState.colors[i * 3];
                gameState.players[i].color.g = msg.payload.fullState.colors[i * 3 + 1];
                gameState.players[i].color.b = msg.payload.fullState.colors[i * 3 + 2];
            }
        }
    }
    ...
    {
        // Send empty connect message - server will assign ID
        std::string msg = serializeConnectMessage(0);
        sendto(clientSocket, msg.c_str(), msg.length(), 0,
               (struct sockaddr*)&serverAddr, sizeof(serverAddr));

        std::cout << "Connect request sent to server" << std::endl;
    }
    ...
    void sendDisconnectRequest()
    {
        ...
        {
            std::string msg = serializeDisconnectMessage(gameState.localPlayerId);
            sendto(clientSocket, msg.c_str(), msg.length(), 0,
                   (struct sockaddr*)&serverAddr, sizeof(serverAddr));

            std::cout << "Disconnect request sent to server" << std::endl;
    ...
    void sendPositionUpdate()
    {
        ...
        std::string msg = serializePositionUpdate(gameState.localPlayerId, gameState.localPosition);
        sendto(clientSocket, msg.c_str(), msg.length(), 0,
               (struct sockaddr*)&serverAddr, sizeof(serverAddr));
    }
    ...
    void processInput()
    {
        ...
        {
            movement.z -= MOVE_SPEED * deltaTime;
        }
        ...
        {
            movement.x -= MOVE_SPEED * deltaTime;
        }
        ...
        {
            movement.y -= MOVE_SPEED * deltaTime;
        }
        ...
        {
            std::lock_guard<std::mutex> lock(gameStateMutex);
            gameState.localPosition += movement;
            gameState.positionChanged = true;

            // Update our position in the players array as well
            if (gameState.localPlayerId > 0 && gameState.localPlayerId <= MAX_PLAYERS)
            {
                gameState.players[gameState.localPlayerId - 1].position = gameState.localPosition;
            }
        }
    }
    ...
    static void frameBufferResizeCallback(GLFWwindow* window, int width, int height)
    {
        ...
        client->framebufferResized = true;
    }
    ...
    static void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
    {
        ...
        if (key >= 0 && key < 348)
        {
            // GLFW_KEY_LAST is usually 348
            if (action == GLFW_PRESS)
            {
                client->keyPressed[key] = true;
            }
            else if (action == GLFW_RELEASE)
            {
                client->keyPressed[key] = false;
            }
        }
    }

    // Vulkan initialization and rendering functions
    void initWindow()
    {
        glfwInit();
        ...
        window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan Mesh Shader Cube Network Demo", nullptr, nullptr);
    ...
    void createInstance()
    {
        ...
        appInfo.pApplicationName = "Mesh Shader Network Cubes";
    ...
    createInfo.pNext = &maintenance4Features;  // Point to the start of our chain
    ...
    // Create swap chain, image views, render pass, descriptor set layout, graphics pipeline
    // ... and all other Vulkan-related implementation from the provided code
    ...
    // Update uniform buffers with player positions
    void updateUniformBuffer(uint32_t currentImage)
    {
        ...
        // Pack camera and projection data
    ...
    // Record drawing commands for each player cube
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
    {
        ...
        // Lock the game state while we draw
        std::lock_guard<std::mutex> lock(gameStateMutex);

        // For each active player, draw a cube
        for (int i = 0; i < MAX_PLAYERS; i++)
        {
            if (gameState.players[i].active)
            {
                // Set player specific data (position and color) via push constants
...
public:
    GameClient(const std::string& serverIp = DEFAULT_SERVER_IP, int serverPort = DEFAULT_SERVER_PORT)
        : running(false)
    {
        ...
        std::cout << "Game Client initialized. Connected to "
    ...
    void start()
    {
        if (!running)
        {
            ...
            networkUpdateThread = std::thread(&GameClient::networkUpdateLoop, this);

            // Send connect request to server
            sendConnectRequest();

            std::cout << "Game Client started" << std::endl;
        }
    }
    ...
    void stop()
    {
        if (running)
        {
            ...
            std::cout << "Game Client stopped" << std::endl;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        GameClient client(serverIP, serverPort);
        client.run();
    }
